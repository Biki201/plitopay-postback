...
// TimeWall allowed IPs
const TIMEWALL_IPS = ['51.81.120.73', '142.111.248.18'];

// IP Whitelist middleware (only for postback route)
function checkTimewallIP(req, res, next) {
  const ip = req.headers['x-forwarded-for']?.split(',')[0].trim() || req.socket.remoteAddress;
  if (!TIMEWALL_IPS.includes(ip)) {
...
// TimeWall Postback endpoint — IP whitelisted
app.get('/timewall-postback', checkTimewallIP, async (req, res) => {
  try {
